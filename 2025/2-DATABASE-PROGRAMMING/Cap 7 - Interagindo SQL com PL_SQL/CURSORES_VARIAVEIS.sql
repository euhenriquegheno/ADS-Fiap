...
DECLARE
    -- Definição do tipo REF CURSOR
    TYPE CURSOR_TYPE IS REF CURSOR;

    -- Variável do tipo REF CURSOR
    CURSOR_VAR CURSOR_TYPE;

    -- Variáveis para armazenar os valores obtidos do cursor
...
BEGIN
    ...
    LOOP
        -- Buscando o próximo registro no REF CURSOR
        FETCH CURSOR_VAR INTO V_NR_CLIENTE, V_VL_MEDIO_VENDAS;

        -- Saindo do loop se não houver mais registros
        EXIT WHEN CURSOR_VAR%NOTFOUND;

        -- Atualizando o valor médio de compras para o cliente atual
        UPDATE DB_CLIENTE
    ...
    -- Atualizando todas as transações pendentes
    COMMIT;

    -- Exibindo o número de linhas processadas
